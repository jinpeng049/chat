...
public class DateUtil {
	...
	public static Month getMonthCn(String date) {
		Month week = null;
		DateStyle dateStyle = getDateStyle(date);
		if (dateStyle != null) {
			Date myDate = StringToDate(date, dateStyle);
			week = getMonthCn(myDate);
		}
		return week;
	}
	/**
	 ...
	 * @return 星期
	 */
	public static Month getMonthCn(Date date) {
		Month week = null;
		Calendar calendar = Calendar.getInstance();
		calendar.setTime(date);
		int weekNumber = calendar.get(Calendar.MONTH);
		switch (weekNumber) {
		case 0:
			week = Month.JANUARY;
			break;
